...
- 路段用时：显示同一天相邻两站之间步行 / 公交地铁 / 驾车的路程和用时，以及每天的合计（配置高德 Key 时由高德路径规划计算，否则按直线距离估算）
...
- 导入 GPX 轨迹：两步路、六只脚、运动手表等导出的 GPX 文件可以直接导入为旅程轨迹
...
**用户体系**
- 游客、用户（Lv1–Lv6，按经验值升级，等级越高存储空间越大）、管理员；用户可以自行注销账号
- 管理后台：数据概览、用户管理（封禁、角色、经验值、重置密码）、内容管理（精选、隐藏、删除、公开旅程审核）、评论管理、举报处理、站点设置（公告、屏蔽词、备案号、用户协议 / 隐私政策）
- 合规：页脚显示 ICP 备案号和公安联网备案号，注册时需同意用户协议和隐私政策
